<template>
  <div class="p-6 space-y-6">
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex flex-wrap gap-4 items-end">
      ...
      <!-- CONDITIONAL: JURUSAN (hanya untuk SMK/SMA/MA) -->
      <div v-if="showJurusanField" class="flex-1 min-w-[200px]">
      ...
      <!-- ========== TAMBAHAN FILTER KELOMPOK MAPEL ========== -->
      <div class="flex-1 min-w-[200px]">
        <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Kelompok Mapel</label>
        <select v-model="filter.kelompok" class="w-full bg-slate-50 border-slate-200 rounded-xl text-sm p-3 outline-none focus:ring-2 focus:ring-indigo-500/20">
          <option value="all">-- Semua Kelompok --</option>
          <option value="A">Kelompok A (Umum Wajib)</option>
          <option value="B">Kelompok B (Muatan Lokal)</option>
          <option v-if="showKejuruanField" value="Kejuruan">Kelompok C (Kejuruan)</option>
        </select>
      </div>
      <!-- ========== SAMPAI SINI ========== -->
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50/50">
              ...
              <!-- ========== TAMPILKAN MAPEL BERDASARKAN KELOMPOK ========== -->
              <th v-for="m in filteredMapels" :key="m.id" class="p-4 text-xs font-bold text-slate-400 uppercase border-b text-center min-w-[100px]">
                {{ m.nama_mapel }}
              </th>
              <!-- ========== SAMPAI SINI ========== -->
          ...
          <tbody>
            <tr v-for="(row, idx) in legerData.data" :key="idx" class="hover:bg-slate-50/80 transition-all">
              ...
              <!-- ========== TAMPILKAN NILAI BERDASARKAN KELOMPOK ========== -->
              <td v-for="m in filteredMapels" :key="m.id" class="p-4 border-b text-center font-mono text-sm">
...
const filter = reactive({ kelas: '', jurusan: '', kelompok: 'all' }) // ← TAMBAH kelompok

const options = reactive({ 
  kelas: [], 
  jurusan: [],
  mapels: [] // ← TAMBAH mapels untuk menyimpan daftar mapel
})
...
// CONDITIONAL: Cek tingkat sekolah untuk jurusan
const showJurusanField = computed(() => {
  const tingkat = auth.tingkat?.toUpperCase() || ''
  return ['SMK', 'SMA', 'MA'].includes(tingkat)
})

// CONDITIONAL: Cek tingkat sekolah untuk kelompok Kejuruan
const showKejuruanField = computed(() => {
  const tingkat = auth.tingkat?.toUpperCase() || ''
  return ['SMK', 'SMA', 'MA'].includes(tingkat)
})

// ========== FILTER MAPEL BERDASARKAN KELOMPOK ==========
const filteredMapels = computed(() => {
  if (filter.kelompok === 'all') {
    return legerData.value.mapels
  }
  return legerData.value.mapels.filter(m => m.kelompok === filter.kelompok)
})
// ========== SAMPAI SINI ==========
...
const fetchLeger = async () => {
  if (!filter.kelas) {
    return Swal.fire('Info', 'Pilih Kelas dulu!', 'warning')
  }

  // Validasi jurusan (jika diperlukan)
  if (showJurusanField.value && !filter.jurusan) {
    return Swal.fire('Info', 'Pilih Jurusan dulu!', 'warning')
  }
  ...
  try {
    ...
    options.mapels = res.data.mapels // ← SIMPAN DAFTAR MAPEL
  } catch (error) {
