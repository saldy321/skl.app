  ...
  <div class="p-6">
      ...
      <div class="flex flex-wrap gap-3">
        ...
        <div class="relative">
          ...
          <input 
            ...
            class="pl-8 pr-8 py-2 border border-slate-300 rounded-lg text-sm w-64 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
          />
          <button 
            v-if="searchKeyword"
            @click="clearSearch"
            class="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-red-500"
          >
        ...
        <button @click="triggerFileInput" :disabled="uploading" class="flex items-center gap-2 bg-[#FF6B4A] hover:bg-[#e85a3d] text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
          <i v-if="uploading" class="fas fa-spinner animate-spin"></i>
          <i v-else class="fas fa-file-excel"></i>
          {{ uploading ? 'Uploading...' : 'Import Excel' }}
        </button>

        <button @click="openPromoteModal" class="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
          <i class="fas fa-graduation-cap"></i>
          Proses Lulus
        </button>

        <button @click="tambahSiswa" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
          <i class="fas fa-plus"></i>
          Tambah
        </button>

        <button 
          @click="openDeleteMassalModal" 
          :disabled="dataSiswa.length === 0"
          class="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors disabled:opacity-50"
        >
    ...
    <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
        ...
        <span class="text-sm text-slate-500">Total: {{ dataSiswa.length }} Siswa</span>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-slate-200 bg-slate-50 text-slate-600 uppercase tracking-wider text-[11px]">
              ...
              <th v-if="showJurusanField" class="p-3 font-bold text-center">Jurusan</th>
          ...
          <tbody class="divide-y divide-slate-100">
            <!-- Loading -->
            <tr v-if="loading">
              <td colspan="showJurusanField ? 8 : 7" class="p-10 text-center">
            ...
            <!-- Data Rows -->
            <tr v-for="(item, index) in dataSiswa" :key="item.id" class="hover:bg-slate-50 transition-colors">
              ...
              <td v-if="showJurusanField" class="p-3 text-center">
                <div class="text-slate-700">{{ item.jurusan || '-' }}</div>
              </td>
              <td class="p-3 text-center">
              ...
              <td class="p-3 text-center">
                <div class="flex justify-center gap-2">
                  <button @click="editSiswa(item)" class="w-8 h-8 rounded-lg bg-orange-50 text-orange-600 hover:bg-orange-600 hover:text-white transition-colors" title="Edit">
                    <i class="fas fa-pencil-alt text-xs"></i>
                  </button>
                  <button @click="deleteSiswa(item.id)" class="w-8 h-8 rounded-lg bg-red-50 text-red-600 hover:bg-red-600 hover:text-white transition-colors" title="Hapus">
    ...
    <div v-if="isEditing || isAdding" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-slate-900/80 backdrop-blur-sm" @click="resetForm"></div>
    ...
    <div v-if="showPromoteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white w-full max-w-lg rounded-xl shadow-2xl overflow-hidden p-0">
    ...
    <!-- MODAL HAPUS MASSAL -->
    <div v-if="showDeleteMassalModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" @click="closeDeleteMassalModal"></div>
      <div class="relative bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden">
        <div class="bg-red-600 p-4 flex justify-between items-center text-white">
          <h3 class="font-bold text-lg flex items-center gap-2"><i class="fas fa-exclamation-triangle"></i> Hapus Semua Data</h3>
          <button @click="closeDeleteMassalModal" class="text-white/80 hover:text-white"><i class="fas fa-times"></i></button>
        </div>
        <div class="p-6 space-y-4">
          <p class="text-sm text-slate-600">
            Anda akan menghapus <span class="font-bold text-red-600">{{ dataSiswa.length }}</span> data siswa 
            untuk tahun lulus <span class="font-bold">{{ auth.selectedYear }}</span>.
          </p>
          <div class="bg-red-50 p-3 rounded-lg border border-red-200">
            <p class="text-xs text-red-700 flex items-center gap-2"><i class="fas fa-info-circle"></i> Data nilai siswa juga akan ikut terhapus!</p>
          </div>
          <div class="flex gap-3">
            <button @click="closeDeleteMassalModal" class="flex-1 px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-100 rounded-lg">Batal</button>
            <button @click="executeDeleteMassal" :disabled="deleting" class="flex-1 px-4 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-lg flex justify-center items-center gap-2">
...
const promoteMode = ref('eligible_only')
...
// State Hapus Massal & Pencarian
const showDeleteMassalModal = ref(false)
...
const editSiswa = (item) => { 
  ...
  form.value = { ...item, tahun_lulus: item.tahun_lulus || '' }
  isEditing.value = true 
}

const saveSiswa = async () => {
  if (!form.value.nisn || !form.value.nama_siswa) {
    return Swal.fire('Warning', 'NISN/Nama Wajib', 'warning')
  }
  try {
    const fd = new FormData()
    Object.keys(form.value).forEach(key => fd.append(key, form.value[key]))
    if (isAdding.value) {
      await api.post(`/${route.params.slug}/admin/siswa`, fd)
    } else {
      await api.put(`/${route.params.slug}/admin/siswa/${selectedId.value}`, fd)
    }
    Swal.fire('Success', 'Disimpan!', 'success')
    fetchSiswa()
    resetForm()
  } catch (err) { 
...
const deleteSiswa = async (id) => {
  ...
  if (res.isConfirmed) { 
    await api.delete(`/${route.params.slug}/admin/siswa/${id}`)
...
const handleFileUpload = async (event) => {
  ...
  try { 
    await api.post(`/${route.params.slug}/admin/siswa/import`, formData)
    Swal.fire('Mantap!', 'Import Berhasil', 'success')
    fetchSiswa() 
  } catch (err) { 
...
const handleSearch = () => {
  ...
  searchTimeout = setTimeout(async () => {
    ...
    try {
      loading.value = true
      const res = await api.get(`/${route.params.slug}/admin/siswa/search`, {
        params: {
...
// ========== LOGIC HAPUS MASSAL ==========
const openDeleteMassalModal = () => {
  if (dataSiswa.value.length === 0) {
    Swal.fire('Info', 'Tidak ada data siswa untuk dihapus', 'info')
    return
  }
  showDeleteMassalModal.value = true
}

const closeDeleteMassalModal = () => {
  showDeleteMassalModal.value = false
}

const executeDeleteMassal = async () => {
  deleting.value = true
  try {
    await api.delete(`/${route.params.slug}/admin/siswa/all`, {
      params: {
        tahun_lulus: auth.selectedYear
      }
    })
    
    Swal.fire('Berhasil!', `Semua data siswa tahun ${auth.selectedYear} telah dihapus`, 'success')
    closeDeleteMassalModal()
    fetchSiswa()
  } catch (err) {
