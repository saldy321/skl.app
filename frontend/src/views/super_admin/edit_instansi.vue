<template>
  <div class="max-w-5xl mx-auto">
    <!-- Header Section -->
    <div class="mb-8">
      <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Instansi</h2>
      <p class="text-sm text-gray-500 mt-1">Perbarui data sekolah dan akun administrator.</p>
    </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="col-span-2 md:col-span-1">
            ...
            <p class="text-[10px] text-gray-500 mt-1">Digunakan sebagai URL login siswa.</p>
          </div>
<div class="col-span-2 md:col-span-1">
  <label class="block text-xs font-semibold text-gray-700 mb-1.5">Kode Instansi</label>
  <input 
    v-model="form.kode_instansi" 
    type="text" 
    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all placeholder-gray-400" 
    placeholder="Contoh: SMKN1CMH"
  >
  <p class="text-[10px] text-gray-500 mt-1">Kode unik untuk identifikasi internal (opsional saat edit).</p>
</div>
          <!-- Tingkat Pendidikan -->
          <div class="col-span-2 md:col-span-1">
      ...
      <div class="flex flex-col sm:flex-row justify-end gap-4 pt-4 border-t border-gray-200">
        <button 
          type="button" 
          @click="$router.push('/super-admin/instansi')" 
...
const fetchInstansiData = async () => {
  try {
    const res = await api.get(`/super/instansi/${route.params.id}`)
    const data = res.data.data
    form.nama_instansi = data.nama_instansi
    form.slug = data.slug
    form.kode_instansi = data.kode_instansi
    form.tingkat_sekolah = data.tingkat_sekolah
    form.alamat = data.alamat
  } catch (err) {
    console.error(err)
    Swal.fire('Error', 'Gagal mengambil data instansi', 'error')
  }
}
...
const handleSubmit = async () => {
  ...
  const payload = {
    ...
    kode_instansi: form.kode_instansi, // Boleh kosong
  ...
  try {
    await api.put(`/super/instansi/${route.params.id}`, payload)
