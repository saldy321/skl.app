<template>
  <div class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" 
       :class="isEnvelopeOpen ? 'bg-gradient-to-br from-slate-800 via-slate-900 to-black' : 'bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50'">
    
    <!-- Background Decoration -->
    <div class="absolute inset-0 z-0 opacity-20 pointer-events-none">
      <div class="absolute top-10 left-10 w-40 h-40 bg-white/30 rounded-full blur-3xl"></div>
      <div class="absolute bottom-10 right-10 w-60 h-60 bg-teal-200/20 rounded-full blur-3xl"></div>
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-emerald-100/10 rounded-full blur-3xl"></div>
    </div>
    ...
    <div v-if="!isEnvelopeOpen && !loading && siswa" 
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-3xl md:text-4xl font-light text-slate-800 tracking-wide">
          Selamat, <span class="font-bold text-emerald-700">{{ siswa?.nama_siswa?.split(' ')[0] }}</span>
        </h1>
      ...
      <div class="mt-10 flex flex-col items-center animate-bounce">
        ...
        <i class="fas fa-fingerprint text-2xl text-emerald-500"></i>
      </div>
    ...
    <transition name="letter-emerge">
      <div v-if="isOpening && !showDocument" 
        ...
        <div class="w-72 h-96 bg-white shadow-2xl rounded-lg p-6 transform rotate-1 animate-float">
          <div class="w-full h-full border border-dashed border-emerald-200 rounded flex flex-col items-center justify-center">
            <div class="w-16 h-16 rounded-full bg-emerald-50 flex items-center justify-center mb-4">
              <i class="fas fa-envelope-open-text text-3xl text-emerald-600"></i>
            </div>
            <p class="text-emerald-700 font-serif text-lg text-center">Membuka Surat Kelulusan...</p>
            <div class="mt-4 flex gap-1">
              <span class="w-2 h-2 bg-emerald-400 rounded-full animate-bounce"></span>
              <span class="w-2 h-2 bg-emerald-400 rounded-full animate-bounce delay-100"></span>
              <span class="w-2 h-2 bg-emerald-400 rounded-full animate-bounce delay-200"></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- STATE 3: DOKUMEN SKL LENGKAP (SESUAI LOGIC ADMIN) -->
...
// --- HELPERS (SAMA PERSIS DENGAN ADMIN) ---
...
// --- LOAD DATA (SESUAI ENDPOINT SISWA) ---
